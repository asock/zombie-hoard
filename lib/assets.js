...
/* ==== home grid ==== */
.home-grid { display: grid; grid-template-columns: 1fr 250px; gap: 24px; }
.home-main { min-width: 0; }
.home-side { min-width: 0; }
.deploy-panel { margin-bottom: 16px; }
.search-panel { margin-bottom: 20px; }
.search-panel .deploy { margin: 0; }

/* ==== compact roster (sidebar) ==== */
.roster-compact { display: grid; gap: 6px; }
.character-card-sm {
  display: flex; align-items: center; gap: 8px;
  background: #0f0518; border: 1px solid #2a1840; border-radius: 4px;
  padding: 8px 10px; text-decoration: none; font-size: 12px;
  transition: all 0.15s ease;
}
.character-card-sm:hover { border-color: var(--color, #c084fc); transform: translateX(2px); }
.character-card-sm .emoji { font-size: 16px; }
.character-card-sm .name { font-weight: 700; letter-spacing: 0.04em; }
.character-card-sm .dim { color: #6a2a8a; font-size: 10px; text-transform: uppercase; letter-spacing: 0.06em; }

/* ==== TOC nav ==== */
.toc { display: flex; flex-wrap: wrap; gap: 6px; margin: 16px 0 24px; }
.toc-item {
  display: inline-flex; align-items: center; gap: 6px;
  background: #0f0518; border: 1px solid #2a1840; border-radius: 4px;
  padding: 6px 12px; font-size: 13px; text-decoration: none;
  color: var(--color, #c39bd3);
  transition: all 0.15s ease;
}
.toc-item:hover { border-color: var(--color, #c084fc); transform: translateY(-1px); }
.toc-item .badge { font-size: 10px; background: #14041e; padding: 2px 6px; border-radius: 3px; color: #6a2a8a; }
...
.exec-summary .content p { margin: 0 0 8px; }

/* ==== badge (reusable inline label) ==== */
.badge {
  display: inline-block; font-size: 10px; padding: 2px 7px; border-radius: 3px;
  background: #14041e; color: #6a2a8a; margin-left: 6px; font-weight: 400; letter-spacing: 0;
}

@media (max-width: 850px) {
  .home-grid { grid-template-columns: 1fr; }
  .home-side { order: -1; }
  .roster-compact { grid-template-columns: repeat(auto-fill, minmax(180px, 1fr)); }
}
...
(function () {
  ...
  // Contextual commentary: character voice + real data.
  function contextualLineClient(zId, sectionClaims, content, research) {
    var cc = sectionClaims.length;
    var gc = sectionClaims.filter(function(c) { return c.grounded; }).length;
    var uc = cc - gc;
    var has = (content || '').length > 50;
    var m = {
      CORTEX: function() {
        if (!has) return 'The drum is empty. The sources had nothing at the core.';
        if (cc === 0) return 'Tarman came up empty.';
      ...
      RELIC: function() {
        if (!has) return 'No historical record. The past was silent.';
        if (cc >= 5) return cc + ' milestones surfaced. Frank remembers.';
        return cc + ' historical claims. ' + gc + ' backed by sources.';
      },
      GEARS: function() {
        if (!has) return 'Ernie opened the hood. Nothing mechanical to find.';
        if (uc === 0) return cc + ' technical claims, all cited. The procedure was correct.';
        return cc + ' claims. Ernie executed the disassembly.';
      },
      VENOM: function() {
        if (!has) return 'Trash found no wounds. Suspiciously clean.';
        if (cc === 0) return 'Nothing to criticize? Check again.';
        return cc + ' failure modes surfaced. ' + (gc ? gc + ' cited.' : 'None cited.');
      },
      CLAW: function() {
        if (!has) return 'Freddy showed up. Nothing to build today.';
        if (gc >= cc && cc > 0) return cc + ' practical claims, all sourced. Step one through ' + cc + '.';
        return cc + ' actionable claims.';
      },
      SPORE: function() {
        var e = (research.exploreNext || []).length;
        if (!has) return 'The gas found no adjacent rooms.';
        if (e >= 3) return cc + ' connections traced. ' + e + ' adjacent topics identified.';
        return cc + ' cross-domain connections. The smoke goes places.';
      },
      PROPHET: function() {
        if (!has) return 'Suicide stared at the horizon. It was blank.';
        if (uc >= cc / 2 && cc > 0) return cc + ' forecasts, mostly uncited. Trajectory analysis.';
        return cc + ' future signals. ' + gc + ' backed by current sources.';
      }
    };
    var fn = m[zId];
    return fn ? fn() : cc + ' claims extracted.';
  }
  ...
  function revealZombieCards(researchData) {
    ...
    roster.forEach(function (z, idx) {
      ...
      if (!content) return;
      var commentary = contextualLineClient(z.id, sectionClaims, content, researchData);
