...
import { CHARACTERS, NECROMANCER, ROSTER, pickLine, getCharacter } from './characters.js';
...
// ── Page: home ───────────────────────────────────────────────────────────────
export function homePage({ graveyard, flash }) {
  const quote = randomLore(HOARD_QUOTES);
  const epitaph = randomLore(GRAVEYARD_EPITAPHS);
  ...
  const topicsHtml = (graveyard.entries || []).length === 0
    ...
    : `<ul class="topic-list">${graveyard.entries.slice(0, 20).map(e => topicListItem(e)).join('')}</ul>`;
  ...
  const body = `
    ...
    <h1>Deploy the Hoard</h1>
    <p class="subtitle">Feed a topic to seven hungry minds. They will return with what they remember.</p>
    ...
    <div class="panel">
      <form class="deploy" method="POST" action="/deploy">
        ...
        <div class="checks">
          <label><input type="checkbox" name="ungrounded" value="1"> Ungrounded mode (skip source fetching)</label>
        </div>
        <div>
    ...
    <h2>🪦 The Graveyard</h2>
    <p class="subtitle">${esc(epitaph)}</p>
    ${topicsHtml}

    <h2>🧟 The Hoard</h2>
    <p class="subtitle">Seven cognitive facets and the man who reads their reports.</p>
...
export function topicPage({ entry, sidecar, content }) {
  ...
  const sourcesHtml = sources.length === 0
    ...
    : `<div class="panel"><h2>📚 Sources Fed to the Hoard</h2><ul class="sources-list">${sources.map(s => `
        <li>
  ...
  const metricsHtml = `<div class="panel"><h2>☠️ Synthesis Report</h2><div class="metrics">
    ...
    ${metric('Runs', String(entry.runCount || 1))}
  ...
  const feedingHtml = ROSTER.map((z) => {
    ...
    const commentary = pickLine(z.onReveal);
    const sectionClaims = claims.filter((c) => c.section === z.id);
    return `<div class="zombie-card revealed" style="--color: ${z.color}; --accent: ${z.accent};">
      <div class="head">
        <div class="portrait-wrap">${z.portrait(z.color)}</div>
        <div class="meta">
          <div class="name">${z.emoji} ${esc(z.name)}</div>
          <div class="title">${esc(z.title)}</div>
          <div class="epitaph">${esc(z.epitaph)}</div>
          <div class="hunger">${esc(z.focus)}</div>
        </div>
      </div>
      ${commentary ? `<div class="commentary">${esc(commentary)}</div>` : ''}
      <div class="content">${markdownLite(sectionContent)}</div>
      ${sectionClaims.length > 0 ? `
        <div class="claims">
          <h4>What ${esc(z.name)} brought back</h4>
          ${sectionClaims.slice(0, 8).map(c => `
            <div class="claim">${esc(c.text)}${c.sources?.length ? `<span class="src">${c.sources.map(i => `[S${i}]`).join(' ')}</span>` : ''}</div>
          `).join('')}
        </div>
      ` : ''}
    </div>`;
  ...
  const exploreHtml = exploreNext.length > 0
    ? `<div class="panel"><h2>🦠 ${esc(randomLore(INFECTION_PHRASES))}</h2><ul class="explore">${exploreNext.map(t => `<li>${esc(t)}</li>`).join('')}</ul></div>`
    : '';
  ...
  const diffHtml = (diff && (diff.added || diff.removed))
    ? `<div class="diff-block"><h3>🩻 Diff vs Prior Run</h3>
        ...
        ${diff.added_claims?.length ? `<h4 class="dim">New since last run:</h4><ul>${diff.added_claims.slice(0, 8).map(c => `<li><span class="dim">(${esc(c.section)})</span> ${esc(c.text)}</li>`).join('')}</ul>` : ''}
        ${diff.removed_claims?.length ? `<h4 class="dim">Dropped since last run:</h4><ul>${diff.removed_claims.slice(0, 8).map(c => `<li><span class="dim">(${esc(c.section)})</span> ${esc(c.text)}</li>`).join('')}</ul>` : ''}
      </div>`
    : '';

  const execSummaryHtml = sidecar?.executive
    ? `<div class="panel"><h2>📝 Executive Summary</h2><div class="content">${markdownLite(sidecar.executive)}</div></div>`
    : '';

  const body = `
    <h1>${esc(topic)}</h1>
    <p class="subtitle">"${esc(necQuote)}" — ${esc(NECROMANCER.title)}</p>
    <p class="subtitle dim">Last raised: ${new Date(entry.lastRun).toLocaleString()} · Run #${entry.runCount}</p>
    ...
    <h2>🧟 The Feeding</h2>
    <p class="subtitle">Each zombie's findings, raised from the report.</p>
    ...
    <div class="panel">
      ...
      <form class="deploy" method="POST" action="/deploy">
        ...
        <button type="submit">🦠 INFECT — re-run and diff</button>
      </form>
    </div>
...
export function characterPage({ id }) {
  const c = getCharacter(id);
  if (!c) return notFoundPage(`Character "${id}" not found`);

  // Find which section in research.js this character corresponds to (so we
  // can show the system prompt-style focus next to their bio)
  ...
  const body = `
    ...
    <div class="bio-section" style="--color: ${c.color};">
      <h2>While reading sources</h2>
      <ul class="voice-list">
        ${(c.onSourceComment || []).map(v => `<li>${esc(v)}</li>`).join('')}
      </ul>
    </div>

    ${c.onClaim ? `
      <div class="bio-section">
        <h2>Trademark claim opener</h2>
        <div class="quote">${esc(c.onClaim)}</div>
      </div>
    ` : ''}
